...
Can a small LLM learn to play Catan? We benchmark frontier models on full
games (via the catanatron engine), then post-train Qwen3.5-9B through
SFT → DAgger → DPO → GRPO to see how far imitation and RL take it.

![game replay](assets/catan_llm_game_replay.gif)
...
Kimi K3, GPT 5.6 Sol, and Fable 5 are the most expensive by far, so we run fewer games of them.
...
#### preliminary: how many games do we need?
...
Overall, the model performance more or less matches what we'd expect: bigger is better (more params), and so is longer (thinking tokens). I'm curious to see how the best models play! See the replay above.

The standout is Sol (thinking), the only model to clear our alpha_beta anchor (8/15 wins vs alpha_beta's 36% — small n though).

MiniMax M3 thinking was too slow (over 100 minutes per game), so we abandoned it before completing its runs.
...
also, we add a mirror matchup to model what happens when models play themselves. run with both d2 and same_turn
...
<details>
<summary><b>preliminary: monte carlo estimation</b></summary>
...
GRPO samples multiple outputs from our *current model*, and treats the average reward of these as a baseline. Later, any result that is greater than this average is a positive "advantage", and anything below is a negative "advantage".

$$A_i = \frac{r_i - \text{mean}(\{r_1, r_2, \cdots, r_G\})}{\text{std}(\{r_1, r_2, \cdots, r_G\})}$$


The advantage is defined as $ A^\pi(s, a) = Q^\pi(s, a) - V^\pi(s). $
...
We can get a sense of how "difficult" a board is for the model by calculating its pass rate. Given $k$ attempts at the same board, what fraction of the games does the model win? We're interested in seeing that over enough samples, the model is capable of winning. Notably, it doesn't have to win every game.


To evaluate the pass rate, we do 8 rollouts over 8 distinct game boards, for a total of 64 games. We use our 9B model finetuned on 10K samples. The sampling temperature is 1.0, to match what the RL trainer will use during its rollouts. This just means the model uses the probabilities learned during training.
...
Perhaps unsurprisingly, this turned out to be too few updates with too sparse a signal for a relatively weak model (low win rate) to be effective. The model won zero games in 800 runs. The mean group reward also barely moved, going from 0.031 to 0.033. There's a fundamental flaw in this setup -- since every decision within a game gets the same advantage, the reward signal is high variance. You need to play many many games (e.g. AlphaGo). We don't have that kind of budget. We thus switched our setup to per decision GRPO.

#### per-decision grpo

Instead of grouping complete games, we form the group at a single decision. The model samples 8 moves at a logged decision, and we score each unique move by rolling out 8 continuations with the engine and averaging the terminal rewards (this is the monte carlo estimation from earlier!). Moves above the group mean get positive advantage. We skip the std normalization from the formula above, since dividing by a tiny std amplifies small gaps into noise.

Our run performed 50 updates, with 32 decision groups of 8 sampled moves per update.

![per-decision GRPO reward](assets/grpo_decision_reward.png)

*Mean group reward per update over 50 per-decision GRPO updates.*

The reward is periodic, and the net integral is ~0.
...
![regret by move type](assets/regret_by_type.png)

*Regret per decision vs the engine-best move, by move type, student vs teacher.*

We can see that both the student (Qwen3.5-9B) and the teacher (alpha-beta) are far from playing the optimal moves! This imposes a ceiling on how good the student can get from being trained on the teacher's decisions. That being said, we haven't reached the ceiling either. We are optimistic that better performance gains can come from scaling up to more decisions, as well as including more reasoning traces for the student to learn, as opposed to merely being told what move was taken.
...
| source | amount | type |
|---|---|---|
| givemeanode | $500 | free credits |
| tinker | $150 | free credits |
| prime intellect | $30 | out of pocket |
